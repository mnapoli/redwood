...
Webhooks enable integrations and complex workflows such as one-to-one automation and data syncing between apps.
Redwood comes with built-in support for receiving and verifying incoming webhooks as well as signing outgoing ones.
...
The third party sending the webhook typically signs the payload with a secret key and expect you to validate the signature before processing it.
...
`@redwoodjs/api` implements many of the most commonly used webhook signature verifiers.
