...
We hope that documenting it well makes it relatively fast and easy.

## Configuring Webpack

To start configuring webpack, run the webpack setup command:

```
yarn rw setup webpack
```

This command adds a file named `webpack.config.js` to your project's `web/config` directory, creating `web/config` if it doesn't exist:
...
`config` is Redwood's webpack configuration, and `mode` is either `'development'` or `'production'`.

Unless you're already familiar with Redwood's webpack configuration, it may be challenging to modify.
You can find Redwood's webpack configurations in [`@redwoodjs/core`](https://github.com/redwoodjs/redwood/tree/main/packages/core/config).
You'll find many there, but the final configuration that ends up being `config` in this function is made by merging the common configuration with another depending on your project's environment (i.e. `mode`).
...
For example, if you'd prefer to go to `example.company.com` instead of `localhost:8910` when you dev on your app locally, instead of opening up the webpack config, just set [`allowedHosts`](https://webpack.js.org/configuration/dev-server/#devserverallowedhosts) and [`host`](https://webpack.js.org/configuration/dev-server/#devserverhost) straight from the CLI:
